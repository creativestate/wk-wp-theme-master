...
function siteorigin_widget_post_selector_enqueue_admin_scripts() {
	if ( ! wp_script_is( 'siteorigin-widget-admin-posts-selector' ) ) {
		...
		wp_enqueue_style( 'siteorigin-widget-admin-posts-selector', plugin_dir_url_custom(SOW_BUNDLE_BASE_FILE).'base/css/post-selector.css', array(), SOW_BUNDLE_VERSION );
		wp_enqueue_script( 'siteorigin-widget-admin-posts-selector', plugin_dir_url_custom(SOW_BUNDLE_BASE_FILE).'base/js/posts-selector' . SOW_BUNDLE_JS_SUFFIX . '.js', array( 'jquery', 'jquery-ui-sortable', 'jquery-ui-autocomplete', 'underscore', 'backbone' ), SOW_BUNDLE_VERSION, true );
...
function siteorigin_widget_post_selector_get_posts_action(){
	...
	foreach($posts->posts as $post) {
		...
		$result['posts'][] = array(
			...
			'id' => $post->ID,
			'thumbnail' => !empty($thumbnail) ? $thumbnail[0] : plugin_dir_url_custom(__FILE__).'../css/img/thumbnail-placeholder.png',
