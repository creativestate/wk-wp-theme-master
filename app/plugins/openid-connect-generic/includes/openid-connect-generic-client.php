...
class OpenID_Connect_Generic_Client {
	 ...
	 * @param $redirect_uri
	 */
	function __construct( $client_id, $client_secret, $scope, $endpoint_login, $endpoint_userinfo, $endpoint_token, $redirect_uri, $settings ){
	...
	function make_authentication_url() {
		$url = sprintf( '%1$s?response_type=code&scope=%2$s&client_id=%3$s&state=%4$s&redirect_uri=%5$s&PartnerIdpId=' . $this->settings->partner_id .'&RequestedACSIdx=4',
			$this->endpoint_login,
