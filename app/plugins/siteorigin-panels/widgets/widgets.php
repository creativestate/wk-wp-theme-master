...
function origin_widgets_enqueue($prefix){
	if($prefix == 'widgets.php') wp_enqueue_script('origin-widgets-admin-script', plugin_dir_url_custom(SITEORIGIN_PANELS_BASE_FILE).'/widgets/js/admin.js', array('jquery'), SITEORIGIN_PANELS_VERSION);
}
	...
	static function get_widget_folders(){
		...
		if(empty($folders)) {
			$folders = array(
				...
				plugin_dir_path(SITEORIGIN_PANELS_BASE_FILE).'widgets/widgets' => plugin_dir_url_custom(SITEORIGIN_PANELS_BASE_FILE).'widgets/widgets',
			);
	...
	static function get_image_folders(){
		...
		if(empty($folders)) {
			$folders = array(
				...
				plugin_dir_path(SITEORIGIN_PANELS_BASE_FILE).'widgets/img' => plugin_dir_url_custom(SITEORIGIN_PANELS_BASE_FILE).'widgets/img',
			);
...
class SiteOrigin_Panels_Widgets_EmbeddedVideo extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if(!wp_script_is('fitvids'))
			wp_enqueue_script('fitvids', plugin_dir_url_custom(SITEORIGIN_PANELS_BASE_FILE).'widgets/js/jquery.fitvids.js', array('jquery'), SITEORIGIN_PANELS_VERSION);

		if(!wp_script_is('siteorigin-panels-embedded-video'))
			wp_enqueue_script('siteorigin-panels-embedded-video', plugin_dir_url_custom(SITEORIGIN_PANELS_BASE_FILE).'widgets/js/embedded-video.js', array('jquery', 'fitvids'), SITEORIGIN_PANELS_VERSION);
...
function siteorigin_panels_video_shortcode($atts){
	...
	$instance = shortcode_atts( array(
		'url' => '',
		'src' => '',
		'poster' => plugin_dir_url_custom(SITEORIGIN_PANELS_BASE_FILE).'video/poster.jpg',
		'skin' => 'siteorigin',
		'ratio' => 1.777,
		'autoplay' => 0,
	), $atts );
